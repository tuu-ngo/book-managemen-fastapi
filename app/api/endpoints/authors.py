...
def create_author(
    ...
    '''Create new author and check unique name'''
...
def update_author(
    ...
    if author_up.name is not None and author_up.name != author.name:
        ...
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail='Another Author with this name name already exists'
            )
